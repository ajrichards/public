...
for (gseIndx in 1:length(gseFiles)){
    ...
    ## convert asthma variable
    severe <- which(targets$disease_state=='Severe Asthma')
    print(paste('severe:',length(severe),sep=" "))
    moderate <- which(targets$disease_state=='Moderate Asthma')
    print(paste('moderate:',length(moderate),sep=" "))
    control <- which(targets$disease_state=='Control')
    print(paste('control:',length(control),sep=" "))

    targets$disease_state[severe] = 'asthma'
    targets$disease_state[moderate] = 'asthma'
    targets$disease_state[control] = 'control'
    ...
    print("...filtering")
    if ('genes.GENE' %in% names(as.data.frame(fitProbes2)) == FALSE){
        print("ERROR: need to specify correct gene id")
        q()
    }
    geneIds = as.character(as.data.frame(fitProbes2)$genes.GENE)
    a <- which(geneIds != "")
